...
# Creating new file
        shutil.copy2('photontestbeamstwo.mac', 'photontest.mac')
      ...
      ### Write rand position of 2nd source###
      # Reading in the file
        with open('photontest.mac', 'r') as file:
          filedata = file.read()

      # Random Number inside source area
        RandPosx2 = round(random.uniform(Arr2[x] - 0.15, Arr2[x] + 0.15), 4)
        RandPosy2 = round(random.uniform(Arr3[y] - 0.15, Arr3[y] + 0.15), 4)

        # Replacing the target string
        a = " "
        j = str(RandPosx2) + a + str(RandPosy2) + a + str(0.0)
        i = str(0.0) + a + str(0.5) + a + str(0.0)
        filedata = filedata.replace(i, j)
    ...
    # Reading in the file
      ...
      # Replacing the target string
        ...
        dforcond2 = math.sqrt(RandPosx2**2 + RandPosy2**2)
      ...
      # Replacing the target string
          ...
          thick = round(
              (0.298 - (math.sqrt(0.45**2 - dfromcenter**2) - (0.45 - 0.16))), 4)
        ...
        if dforcond2 <= 0.344:
          ...
          with open('photontest.mac', 'r') as file:
            filedata = file.read()
      # Replacing the target string
          dfromcenter2 = math.sqrt(RandPosx2**2 + RandPosy2**2)

          thick2 = round(
              (0.298 - (math.sqrt(0.45**2 - dfromcenter2**2) - (0.45 - 0.16))),
              4)
          middle2 = -(0.298 - thick2) / 2
          a = " "
          j = a + str(thick2 / 2) + a
          i = a + str(0.1490) + a
          filedata = filedata.replace(i, j)
          ...
          a = " "
          r = str(RandPosx2) + a + str(RandPosy2) + a + str(middle2)
          e = str(RandPosx2) + a + str(RandPosy2) + a + str(0.0)
          filedata = filedata.replace(e, r)
          ...
          numphoton2 = int(math.floor((6677 / 0.3) * thick2))
          a = " "
          j = a + str(numphoton2)
          i = a + str(6676)
          #6736 (10,000) 7410 (11,000)
        ...
        if w == 2000:
          ...
          while any("0 idle, 0 running, 0 held" not in s for s in text):
            ...
            if any("0 idle, 0 running, 0 held" in s for s in text):
              ...
              p.wait()
              hadd = [
                  "hadd Total[%s][%s].root Analysis*.root" % (Arr2[x], Arr3[y])
              ]
